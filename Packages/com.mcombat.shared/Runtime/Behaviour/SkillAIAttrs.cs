...
public class SkillAIAttrs
{
    ...
    static Action<IList<Row>> applyOverrides;
    static bool searchedForOverrides;
    ...
    public static void Load(TextAsset csv)
    {
        ...
        ApplyOverridesIfAvailable(rowList);
        isLoaded = true;
    }

    static void ApplyOverridesIfAvailable(IList<Row> rows)
    {
        if (!searchedForOverrides)
        {
            searchedForOverrides = true;
            applyOverrides = ResolveApplyOverrides();
        }

        applyOverrides?.Invoke(rows);
    }

    static Action<IList<Row>> ResolveApplyOverrides()
    {
        var assemblies = AppDomain.CurrentDomain.GetAssemblies();
        for (var i = 0; i < assemblies.Length; i++)
        {
            var tunerType = assemblies[i].GetType("SkillAIDistanceAutoTuner");
            var method = tunerType?.GetMethod("ApplyOverrides", BindingFlags.Public | BindingFlags.Static);
            if (method == null)
            {
                continue;
            }

            return rows => method.Invoke(null, new object[] { rows });
        }
