      ...
      private static void PreferencesGUI()
      {
         ...
         switch (tab)
         {
            case 0:
            {
               ...
               {
                  if (EditorUtility.DisplayDialog("Reset configuration?", $"Reset the configuration of {Crosstales.BWF.Util.Constants.ASSET_NAME}?", "Yes", "No"))
                  {
                     Crosstales.BWF.Util.Config.Reset();
                     EditorConfig.Reset();
                     save();
                  }
               }
